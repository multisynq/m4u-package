...
using Callback = System.Action<string>;
  ...
  public class UnitySendMessageDispatcher
  {
    public static void Dispatch(string name, string method, string message) {
      ...
      if (obj != null) obj.SendMessage(method, message);
    }
  }
#endif

public class WebViewObject_WebGLTolerant : MonoBehaviour {
  ...
  public void SetKeyboardVisible(string keyboardVisibleHeight) {
    if (BottomAdjustmentDisabled()) return;
  ...
  public bool IsKeyboardVisible {
    ...
    private static extern void _CWebViewPlugin_SetVisibility(IntPtr instance, bool visibility);
    [DllImport("__Internal")]
    private static extern void _CWebViewPlugin_SetScrollbarsVisibility(IntPtr instance, bool visibility);
    [DllImport("__Internal")]
    ...
    [DllImport("__Internal")]
    private static extern void _CWebViewPlugin_SetInteractionEnabled(IntPtr instance, bool enabled);
    [DllImport("__Internal")]
    private static extern bool _CWebViewPlugin_SetURLPattern(IntPtr instance, string allowPattern, string denyPattern, string hookPattern);
    [DllImport("__Internal")]
    private static extern void _CWebViewPlugin_LoadURL(IntPtr instance, string url);
  ...
  public void SetVisibility(bool v) {
    #if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
      if (bg != null) bg.gameObject.SetActive(v);  //if (bg != null) bg.gameObject.active = v;
    #endif
    if (GetVisibility() && !v) {
  ...
  public bool SetURLPattern(string allowPattern, string denyPattern, string hookPattern) {
      ...
      if (webView == IntPtr.Zero) return false;
  ...
  public int Progress() {
      ...
      return 0;
    #elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IPHONE
      if (webView == IntPtr.Zero) return 0;
  ...
  public void SetTextZoom(int textZoom) {
    ...
    void Update() {
      ...
      if (webView == IntPtr.Zero || !visibility) return;
    ...
    void OnGUI() {
      ...
      case EventType.ScrollWheel:
        if (hasFocus) {
          ...
          {
            int mouseState = 0;
            if      (Input.GetButtonDown("Fire1")) mouseState = 1;
            else if (Input.GetButton("Fire1"))     mouseState = 2;
            else if (Input.GetButtonUp("Fire1"))   mouseState = 3;
