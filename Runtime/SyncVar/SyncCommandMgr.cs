...
public class SyncCommandMgr : JsCodeInjectingMonoBehavior {
  ...
  public void ExecuteCommand(string commandId, params object[] parameters) {
    ...
    var msg = $"{syncCommands[commandId].commandIdx}{msgSeparator}{commandId}{msgSeparator}{serializedParams}";
    Debug.Log($"{scLogPrefix} <color=#ff22ff>SEND</color> msg:'<color=cyan>{msg}</color>' for command <color=cyan>{syncCommands[commandId].commandIdx}</color>|<color=white>{commandId}</color>");
