...
public class  Mq_Spatial_System : Mq_System {
  ...
  public Dictionary<int, Mq_Comp> GetComponents() {
    return components;
  }

  private Stopwatch stopWatch;

  public static Mq_Spatial_System Instance { get; private set; }
  private void Awake() {
  ...
  public override List<string> InitializationStringsForObject(GameObject go) {
    ...
    Mq_Spatial_Comp sc = go.GetComponent<Mq_Spatial_Comp>(); // if any
  ...
  private string PackedOptionValues(Mq_Spatial_Comp spatial) {
  ...
  private void UnpackOptionValues(string packedValues, Mq_Spatial_Comp spatial) {
  ...
  private void Update() {
    ...
    if (Mq_Bridge.Instance.unitySceneState == "running") {
      UpdateTransforms();
    }
  }

  void Unparent(string[] args) {
    GameObject child = Mq_Entity_System.Instance.GetGameObjectByCroquetHandle(int.Parse(args[0]));
    if (child) {
  ...
  void SetParent(string[] args) {
    GameObject child = Mq_Entity_System.Instance.GetGameObjectByCroquetHandle(int.Parse(args[0]));
    GameObject parent = Mq_Entity_System.Instance.GetGameObjectByCroquetHandle(int.Parse(args[1]));
    if (parent && child) {
  ...
  public override void PawnInitializationComplete(GameObject go) {
    ...
    if (manifest != null && Array.IndexOf(manifest.mixins, "Ballistic2D") >= 0) {
      ...
      // Mq_Bridge.Instance.Log("info", $"spatial setup for {go}");
    }
  }
  ...
  public override void ActorPropertySet(GameObject go, string propName) {
    ...
    if (propName == "ballisticVelocity") {
      ...
      // Mq_Bridge.Instance.Log("info", $"set ballisticVelocity for {go} (magnitude {velocity.magnitude})");
    }
  }
  ...
  void UpdateTransforms() {
    foreach (KeyValuePair<int, Mq_Comp> kvp in components) {
      Mq_Spatial_Comp spatial = kvp.Value as Mq_Spatial_Comp;
      ...
      if (spatial.ballisticVelocity != null) {
        ...
        if (spatial.currentLag < spatial.desiredLag) {
          ...
          // if (spatial.currentLag >= spatial.desiredLag) Mq_Bridge.Instance.Log("info","hit desired lag");
        ...
        if (Mathf.Abs(offToSide) > 0.01f) {
          ...
          // Mq_Bridge.Instance.Log("info", $"nudge by {nudge.ToString()}");
        }
  ...
  void UpdateSpatial(byte[] rawData, int startPos) {
    ...
    while (bufferPos < rawData.Length) {
      ...
      int instanceID = Mq_Entity_System.GetInstanceIDByCroquetHandle(croquetHandle);

      Mq_Spatial_Comp spatialComponent;
      Transform trans;
      try {
        spatialComponent = components[instanceID] as Mq_Spatial_Comp;
      ...
      if ((encodedId & POS_ANY) != 0) {
        ...
        if (trackedObject != null && spatialComponent.gameObject == trackedObject) {
          ...
          Mq_Bridge.Instance.Log("session",$"after {deltaTMS}ms: d={dist:F3} implying v={v:F3}");
        }
  ...
  public void DrivePawn(int croquetHandle, Vector3? position = null, Quaternion? rotation = null, Vector3? scale = null) {
    ...
    Mq_Spatial_Comp spatial = components[instanceID] as Mq_Spatial_Comp;
  ...
  public void DriveActor(int croquetHandle, bool useSnap, Vector3? position = null, Quaternion? rotation = null,
    ...
    Mq_Bridge.Instance.SendThrottledToCroquet($"objectMoved_{croquetHandle}", argList.ToArray());
  }
  ...
  // 8   0   4
  // 7   6   5
  float[,] allQuadrants = { { 0, 0 },       // centre { -1f, 1f },    // -x+z "1" { 0, 1f },      // +z   "2" { 1f, 1f },     // +x+z "3" { 1f, 0 },      // +x   "4" { 1f, -1f },    // +x-z "5" { 0, -1f },     // -z   "6" { -1f, -1f },   // -x-z "7" { -1f, 0 },     // -x   "8"
  };
  ...
  private void HandleRenderForSpatials(bool isPreRender) {
    ...
    foreach (Mq_Comp c in Mq_Spatial_System.Instance.GetComponents().Values) {
      Mq_Spatial_Comp spatial = c as Mq_Spatial_Comp;
      if (isPreRender) {
